...
The ESI HV plugin is at `esibd/devices/esi/esi.py` (P6.6): INPUTDEVICE, unit V, ONE controller with a device-level COM Port setting (the ESI-CTRL DLL is single-instance per process — the esibd_bs class guards connect; never run the plugin and an ESI notebook together). Channel = HV module address 0–3 (lab uses 2+3); On/Off logic maps to controller + module activation states; per-channel read-only current indicator (nA). Its hk piggyback pushes `Temp_CPU`/`Temp_PSU`/`Activated` (not `Enabled` — bring-up always enables, activation is the meaningful state).
