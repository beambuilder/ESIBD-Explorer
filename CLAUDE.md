...
Since 2026-07-13 all voltage sets funnel through ONE apply-worker queue (`applyValueFromThread` override — the framework's thread-per-channel apply starved the serial lock with many channels and silently dropped sets), and `toggleOn` reverts the On toggle via `revertOnSignal` if any unit's PSU enable fails (never pretend On).
The ESI HV plugin is at `esibd/devices/esi/esi.py` (P6.6; reworked for firmware 1-00, P9 session 3, 2026-07-27): INPUTDEVICE, ONE controller with a device-level COM Port setting (the ESI-CTRL DLL is single-instance per process — the esibd_bs class guards connect; never run the plugin and an ESI notebook together; since the 1-10 package of 2026-09-22 esibd_bs loads the x64 DLL directly — the 1-00-era msl-loadlib bridge, with its few extra seconds on first connect, is now only the `DLL_FLAVOR = "x86"` fallback). Channel = HV module address 0–3 (lab uses 2 = inlet, 3 = emitter). Channel VALUE sets the target voltage (V), but the device unit is **nA** and the plotted/recorded channel data is the measured module output current (`HVChannel.appendValue` override, user 2026-07-06) — the voltage readback stays as the monitor (`U (V)` column + deviation warning) and is what `_pushTelemetry` logs to telemetry.db (unit pinned to 'V' there). **1-00 semantics (device-level activation API is GONE; operation is NVM-config-driven): On = `load_current_config(<Config column selection>)`** — a working config takes effect immediately (heater + HV), no enable step; selecting a Config while On loads it the same way, while Off it is only stored. **The config is the truth (user 2026-07-27):** after every load the channel voltages and the `Heater temperature` device setting sync FROM the device (SW-plugin sync mechanics: `workingSetSyncSignal` → `syncInputs`, `controller.syncing` + `lastAppliedValue` guards), then edits write through live; modules of unchecked rows are deactivated on top of the config. **Park target = the Off config (slot 0, `OFF_CONFIG`; user decision 2026-07-28, supersedes Standby slot 1 of 2026-07-27) at ALL park points: right after init (`_parkAtInit` immediately after `connect()` — the esibd_bs bring-up's `set_enable(True)` leaves the device enabled; esibd_bs untouched, the brief transient is accepted), on Enable-off and on close**, with a loud `set_enable(False)` fallback ("PARK NOT CONFIRMED") — the Config dropdown is deliberately NOT snapped on park (the next On loads the user's selection). The dropdown default/fallback stays Standby slot 1 (`STANDBY_CONFIG`) — a fallback to 'Off' would make Enable a silent no-op (the SW plugin's historical bug). The per-channel `Config` COMBO is mirrored across all real rows (one controller); dropdown mechanics = PSU plugin (cache `esi_nvm_configs.json`, enumeration at every init via curated `list_configs()`/`get_config_name()`, re-match by index, List-configs toolbar action; **enumerate the ACTIVE slots, never the valid ones** — valid = active + deleted, and the controller keeps deleted configs recoverable, so valid resurrects the previous config set as ghosts (fixed 2026-09-23); every enumeration **replaces** the cache file rather than merging into it, so a refresh overwrites the list completely; in Test Mode the esibd_bs sim models the NVM, so real enumeration runs). **Save toolbar action** (`saveconfig.png`, since v1.2): SW-precedent dialog (slot 2–1022 — slots 0/1 protected; name ≤ 201 chars; overwrite shown) → `save_current_config` + `set_config_name` with `NVM_SETTLE_S` waits (skipped in Test Mode) → re-enumeration refreshes the dropdown; **requires On** — a parked device would store the Off working set; no `call_with_retry` (DLL bridge, not serial-family) and no Test-Mode branch (the sim models the NVM end-to-end). Heater (HTCTRL-24-10, address 0): `Heater temperature` setting applies live (negative = control off); a manual target below 50 °C sets the 20 W power limit first (overshoot rule; the setter is not simulated → skipped in Test Mode; a failed limit set ABORTS the target set); measured temp shows in the read-only `Heater T measured` indicator setting (read via `get_heat_ctrl_monitoring` each cycle). Its hk piggyback pushes `Temp_CPU`/`Temp_PSU`/`Activated` (since 1-00 derived from `get_main_state() == STATE_ON` — channel name kept for the dashboard) plus `Temp_Heater` (valid reads only).
